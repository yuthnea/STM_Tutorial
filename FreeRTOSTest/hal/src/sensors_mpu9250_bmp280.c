...
static bool isMpu9250TestPassed = false;
...
static void sensorsDeviceInit(void)
{
	...
	if ( mpu9250TestConnection() == false ){
		while(1);
	}
	vTaskDelay(100);
	mpu9250Init();
...
bool sensorsMpu9250Bmp280Test(void)
{
	  ...
	  testStatus &= isMpu9250TestPassed;
...
bool sensorsMpu9250Bmp280ManufacturingTest(void)
{
	...
	testStatus = mpu9250SelfTest();
	if (testStatus)
	{
		sensorsBiasObjInit(&gyroBiasRunning);
		while (xTaskGetTickCount() - startTick < SENSORS_VARIANCE_MAN_TEST_TIMEOUT)
		{
			mpu9250GetMotion6(&a.y, &a.x, &a.z, &g.y, &g.x, &g.z);
...
void sensorsMpu9250Bmp280SetAccMode(accModes accMode)
{
  switch (accMode)
  {
    case ACC_MODE_PROPTEST:
    	mpu9250SetAccelDLPF(MPU9250_ACCEL_DLPF_BW_460);
      for (uint8_t i = 0; i < 3; i++)
      {
    ...
    default:
    	mpu9250SetAccelDLPF(MPU9250_ACCEL_DLPF_BW_44);
      for (uint8_t i = 0; i < 3; i++)
      {
        lpf2pInit(&accLpf[i],  1000, ACCEL_LPF_CUTOFF_FREQ);
      }
